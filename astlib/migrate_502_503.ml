...
and copy_function_param_desc :
  Ast_502.Parsetree.function_param_desc -> Ast_503.Parsetree.function_param_desc =
  function
  | Ast_502.Parsetree.Pparam_val (l,e,p) ->
    Ast_503.Parsetree.Pparam_val ( copy_arg_label l
                                 , Option.map copy_expression e
                                 , copy_pattern p)
...
and copy_function_body :
  ...
  function
  | Ast_502.Parsetree.Pfunction_body e -> Ast_503.Parsetree.Pfunction_body (copy_expression e)
  | Ast_502.Parsetree.Pfunction_cases (cases, loc, attributes) ->
    Ast_503.Parsetree.Pfunction_cases (List.map copy_case cases, copy_location loc, copy_attributes attributes)

and copy_type_constraint :
  Ast_502.Parsetree.type_constraint -> Ast_503.Parsetree.type_constraint =
  function
  | Ast_502.Parsetree.Pconstraint t -> Ast_503.Parsetree.Pconstraint (copy_core_type t)
  | Ast_502.Parsetree.Pcoerce (t1,t2) -> Ast_503.Parsetree.Pcoerce (Option.map copy_core_type t1, copy_core_type t2) 
...
and copy_value_constraint : Ast_502.Parsetree.value_constraint -> Ast_503.Parsetree.value_constraint
    =
    function
    | Ast_502.Parsetree.Pvc_constraint {
      ...
      Ast_503.Parsetree.Pvc_constraint {
        locally_abstract_univars = List.map (copy_loc (fun x -> x)) locally_abstract_univars;
        typ = copy_core_type typ;
      }
    | Ast_502.Parsetree.Pvc_coercion { ground; coercion} ->
      Ast_503.Parsetree.Pvc_coercion { ground = Option.map copy_core_type ground;
...
and copy_core_type_desc :
  ...
  | Ast_502.Parsetree.Ptyp_open (x0,ty) ->
      Ast_503.Parsetree.Ptyp_open (copy_loc copy_Longident_t x0,copy_core_type ty)
